...
public class CompressImageUtils {
    ...
    public static void compressImageFile(CropParams cropParams, Uri originUri, Uri compressUri) {
        ...
        try {
            ...
            options.inSampleSize = computeSampleSize(options, minSideLength, cropParams
                    .compressWidth * cropParams.compressHeight);
            ...
            boolean result = bitmap.compress(Bitmap.CompressFormat.JPEG, cropParams
                    .compressQuality, out);
    ...
    private static int computeInitialSampleSize(BitmapFactory.Options options, int minSideLength,
        ...
        int lowerBound = (maxNumOfPixels == -1) ? 1 : (int) Math.ceil(Math.sqrt(w * h /
                maxNumOfPixels));
        int upperBound = (minSideLength == -1) ? 128 : (int) Math.min(Math.floor(w /
                minSideLength), Math.floor(h / minSideLength));
