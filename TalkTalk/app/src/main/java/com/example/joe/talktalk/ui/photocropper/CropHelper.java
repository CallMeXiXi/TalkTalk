package com.example.joe.talktalk.ui.photocropper;
 ...
 * - 12:20 2014/10/04 Add "scaleUpIfNeeded" crop options for scaling up cropped images if the
 * size is too small.
...
public class CropHelper {
    ...
    public static Uri generateUri() {
        File cacheFolder = new File(Environment.getExternalStorageDirectory() + File.separator +
                CROP_CACHE_FOLDER);
        if (!cacheFolder.exists()) {
            try {
                boolean result = cacheFolder.mkdir();
                Log.d(TAG, "generateUri " + cacheFolder + " result: " + (result ? "succeeded" :
                        "failed"));
            } catch (Exception e) {
    ...
    public static void handleResult(CropHandler handler, int requestCode, int resultCode, Intent
            data) {
        if (handler == null)
            return;
                    ...
                    if (isPhotoReallyCropped(cropParams.uri)) {
                        ...
                        if (context != null) {
                            if (data != null && data.getData() != null) {
                                String path = CropFileUtils.getSmartFilePath(context, data
                                        .getData());
                                boolean result = CropFileUtils.copyFile(path, cropParams.uri
                                        .getPath());
                                if (!result) {
    ...
    public static boolean clearCacheDir() {
        ...
        if (cacheFolder.exists() && cacheFolder.listFiles() != null) {
            for (File file : cacheFolder.listFiles()) {
                ...
                Log.d(TAG, "Delete " + file.getAbsolutePath() + (result ? " succeeded" : " " +
                        "failed"));
            }
            return true;
        }
